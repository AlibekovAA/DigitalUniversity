...
В конце  файла  [`db/initdb.sql`](db/initdb.sql)  есть пример заполнения. Для заполенения поля `usermax_id` используется `user_id` пользователя на платформе MAX, который можно получить:
```
cd script/
go run .
```
